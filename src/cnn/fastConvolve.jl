...
function fastConvolve!(cLayer::Conv1D, Ai::AbstractArray)
    ...
    Z = cLayer.Z = vec2img1D(cLayer.K*img2vec(Ai), cLayer.outputS) .+ cLayer.B[:,1,:]
...
function fastConvolve!(cLayer::Conv2D, Ai::AbstractArray)
    ...
    Z = cLayer.Z = vec2img2D(cLayer.K*img2vec(Ai), cLayer.outputS) .+ cLayer.B[:,:,1,:]
...
function fastConvolve!(cLayer::Conv3D, Ai::AbstractArray)
    ...
    Z = cLayer.Z = vec2img3D(cLayer.K*img2vec(Ai), cLayer.outputS) .+ cLayer.B[:,:,:,1,:]
