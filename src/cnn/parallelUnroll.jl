...
function unroll(cLayer::Conv1D, AiS::Tuple, param::Symbol=:W)
    ...
    T = eltype(W)
    B = cLayer.B
    k2d = Array{Array{T,1},1}(undef,ci)
    k = Array{Array{T,1},1}(undef, n_H)
    K = Array{Array{T,2},1}(undef, c)
    # Threads.@threads
    for ch=1:size(W)[end]
        w3d = W[:,:,ch]
        # @simd
...
function unroll(cLayer::Conv2D, AiS::Tuple, param::Symbol=:W)
    ...
    T = eltype(W)
    B = cLayer.B
    k2d = Array{Array{T,1},1}(undef,n_Hi)
    k3d = Array{Array{T,1},1}(undef, ci)
    k4d = Array{Array{T,1},1}(undef, n_W)
    k = Array{Array{T,2},1}(undef, n_H)
    K = Array{Array{T,2},1}(undef, co)
    # @simd
    for ch=1:size(W)[end]
        w3da = W[:,:,:,ch]
        for i=range(1, step=s_H, length=n_H)
